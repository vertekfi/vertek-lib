...
export function calculateInvariant(
  ...
  fpRawWeights: BigNumberish[],
): BigNumber {
  const normalizedWeights = fpRawWeights.map(fromFp);
  const balances = fpRawBalances.map(decimal);
  const invariant = balances.reduce(
    (inv, balance, i) => inv.mul(balance.pow(normalizedWeights[i])),
    decimal(1),
  );
  return bn(invariant);
}
...
export function calcBptOutGivenExactTokensIn(
  ...
  for (let i = 0; i < balances.length; i++) {
    ...
    invariantRatioWithFees = invariantRatioWithFees.add(
      balanceRatiosWithFee[i].mul(weights[i]),
    );
  }
  ...
  for (let i = 0; i < balances.length; i++) {
    ...
    const tokenBalanceRatio = balances[i]
      .add(amountInWithoutFee)
      .div(balances[i]);
...
export function calcTokenInGivenExactBptOut(
  ...
  const amountIn = amountInAfterFee.div(
    decimal(1).sub(tokenBalancePercentageExcess.mul(swapFeePercentage)),
  );
  return fp(amountIn);
}
...
export function calcBptInGivenExactTokensOut(
  ...
  let invariantRatio = decimal(1);
  for (let i = 0; i < balances.length; i++) {
    const tokenBalancePercentageExcess = weightedBalanceRatio.lte(
      balanceRatiosWithoutFee[i],
    )
      ? 0
      : weightedBalanceRatio
          .sub(balanceRatiosWithoutFee[i])
          .div(decimal(1).sub(balanceRatiosWithoutFee[i]));

    const amountOutBeforeFee = amountsOut[i].div(
      decimal(1).sub(swapFeePercentage.mul(tokenBalancePercentageExcess)),
    );
    const tokenBalanceRatio = decimal(1).sub(
...
export function calcTokenOutGivenExactBptIn(
  ...
  const swapFeePercentage = fromFp(fpSwapFeePercentage);
  const weight = fromFp(fpWeights[tokenIndex]);
  const balance: any = fpBalances.map(fromFp)[tokenIndex];
  ...
  const amountOut = amountOutBeforeFee.mul(
    decimal(1).sub(tokenBalancePercentageExcess.mul(swapFeePercentage)),
  );
  return fp(amountOut);
}
...
export function calculateBPTSwapFeeAmount(
  ...
  const swapFeePercentage = FP_100_PCT.sub(
    fpDiv(supplyGrowthRatio, fpInvariantGrowthRatio),
  );
...
export function calculateMaxOneTokenSwapFeeAmount(
  ...
  const exponent = decimal(1).div(weight);
  const maxAccruedFees = balance.mul(
    decimal(1).sub(fromFp(fpMinInvariantRatio).pow(exponent)),
  );
...
export function calculateBPTPrice(
  ...
  return bn(
    toFp(
      fromFp(fpBalance).div(fromFp(fpWeight)).div(fromFp(totalSupply)),
    ).toFixed(0),
  );
}
